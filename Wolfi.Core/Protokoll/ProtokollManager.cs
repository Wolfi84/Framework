...
namespace Wolfi.Core.Protokoll
{
     /// <summary>
     /// Stellt einen Dienst zum Verwalten eines
     /// Anwendungsprotokolls bereit.
     /// </summary>
    public class ProtokollManager : Core.Kontext.Anwendungsobjekt
    {
        /// <summary>
        ...
        /// Internes Feld für die Eigenschaft.
        /// </summary>
        private Kontext.SchwacherMethodenVerweisListe _Rückrufe = null;

        /// <summary>
        /// Ruft die Liste zum Sammeln der abonnierten
        /// Rückmethoden ab.
        /// </summary>
        protected Kontext.SchwacherMethodenVerweisListe Rückrufe
        {
            get
            {
                if (this._Rückrufe == null)
                {
                    this._Rückrufe = new Kontext.SchwacherMethodenVerweisListe();
                    this.Eintragen(
        ...
        {
            this.Rückrufe.Add(new Kontext.SchwacherMethodenVerweis(rückrufMethode));
        ...
        /// <remarks>Sollten beim Speichern 10mal Fehler auftreten,
                            ...
                            Schreiber.WriteLine(
                                string.Format(
                                    ...
                                    eintrag.Text.Replace("\r\n", " ").Replace("\t", " ")                       //{2}
                                                                                                               //        ^-> Beim Exportieren eines Textes in eine
                                                                                                               //            CSV darf kein Zeilenumbruch und das
                                                                                                               //            benutzte Trennzeichen in der Ausgabe vorkommen
                                    ));
            ...
            get
            {
                if (!this._MaximaleLängeProtokollTyp.HasValue)
                {
                    this._MaximaleLängeProtokollTyp
                        = (from e in System.Enum.GetNames(typeof(Core.Protokoll.ProtokollEintragTyp))
                           select e.Length).Max();
